...
BLACK, RED, GREEN, BLUE, WHITE = '#000000', '#FF0000', '#00FF00', '#0000FF', '#FFFFFF'
####################################################################################################
...
def bracket_match(ipt: str) -> list[bool, list[int]]:   # TODO: 适配上下限值
...
def simplify_colored_opt(strings: list[str], colors: list[str]) -> tuple[list[str], list[str]]:
    '''默认两个输入长度相同且非空'''
    pin = 0
    while(pin + 1 < len(strings)):
        if colors[pin] == colors[pin + 1]:
            strings[pin] = strings[pin] + strings[pin + 1]
            strings.pop(pin + 1)
            colors.pop(pin + 1)
        else: pin += 1
    return (strings, colors)
def decide_extreme(value: int, minvalue: int, maxvalue: int) -> int:
    ...
    if value >= succ: return 1
    elif value <= fail: return -1
    return 0
####################################################################################################
...
def special(ipt: str) -> tuple[list, list[str]]:
    ...
    match order:
        ...
        case 'POK':
            opt_value = sample(poker_cards, roll_num)
            opt_strings = ['随机扑克牌:', ','.join(opt_value)]
        case 'MAJ':
            opt_value = sample(majang_cards, roll_num)
            opt_strings = ['随机麻将牌:', ','.join(opt_value)]
        case 'GUA':
            gua8_us, gua8_ds = [randint(0, 7) for _ in range(roll_num)], [randint(0, 7) for _ in range(roll_num)]
            gua64_names, gua64_ranks = [gua_64[u][d] for u, d in zip(gua8_us, gua8_ds)], [gua_64_rank[u][d] for u, d in zip(gua8_us, gua8_ds)]
            gua64_unicodes = [gua_64_unicode[rank - 1] for rank in gua64_ranks]
            opt_value = [(gua_8[u], gua_8[d], name, rank, uni) for u, d, name, rank, uni in zip(gua8_us, gua8_ds, gua64_names, gua64_ranks, gua64_unicodes)]
            opt_strings = ['随机六十四卦:']
            opt_strings += [f'上{uc}下{dc}，为{name}卦({rank},{uni})' for uc, dc, name, rank, uni in opt_value]
            if roll_num > 1: opt_strings.append('Results: | ' + ' | '.join([f'{name}({rank},{uni})' for _, _, name, rank, uni in opt_value]) + ' |')
        case 'TLL':
            opt_value = [f'{a_card}{'(正)' if randint(0, 1) else '(逆)'}' for a_card in sample(large_arcana, roll_num)]
            opt_strings = ['随机塔罗牌:', ','.join(opt_value)]
        case 'TLA':
            opt_value = [f'{a_card}{'(正)' if randint(0, 1) else '(逆)'}' for a_card in sample(total_arcana, roll_num)]
            opt_strings = ['随机塔罗牌:', ','.join(opt_value)]
...
def nopro_special(ipt: str) -> list[tuple[int] | str]:
    opt_value = []
    _, order, roll_num = iflegal_special(ipt)
    match order:
        ...
        case 'TLL': opt_value = [f'{a_card}{'(正)' if randint(0, 1) else '(逆)'}' for a_card in sample(large_arcana, roll_num)]
        case 'TLA': opt_value = [f'{a_card}{'(正)' if randint(0, 1) else '(逆)'}' for a_card in sample(total_arcana, roll_num)]
    return opt_value
...
extreme_mode: int = 0
logs_strings, logs_types, log_num = [], [], 0
# 程序控制区
...
def export_log():
    ...
    with open('__logs__\\' + log_name, 'w', encoding = 'utf-8') as log_file:
        ...
        for log_id in range(log_num):
            log_file.write(f'log {log_id + 1}' if log_id in output_ranks_list else f'log {log_id + 1} (deleted)')
            log_file.write(f': {logs_types[log_id]}\n')
...
def protect(function_to_protect):
    '''窗口保护修饰器'''
...
control_names = ['display', 'log', 'extreme']
control_texts = ['详细', 'LOG', '无大成失']
control_funcs = [display_mode_switch, export_log, extreme_mode_switch]
...
def output_totext(ipt_strings: list[str], ifblank: bool):
    ...
    seed(time())
    text_opt.see(END)
def output_totext_colored(ipt_stringss: list[list[str]], colorss: list[list[str]]): # 还未测试
    ...
    seed(time())
    text_opt.see(END)
def output_create_record(lines: int, value: int | str | list | tuple, otype: str, ifblank: bool):
    '''记录输出内容'''
...
@protect
def show_result_pro_base(ipt: str, code: str):
    ...
    match code: # 输出文本特殊处理, 包含描述的插入
        ...
        case 'special':
            ...
            match order:
                ...
                case 'POK':
                    if roll_num == 1 and bracket_num != 1: strings_opt[0] = strings_opt[0] + describe
                    elif roll_num == 1 and bracket_num == 1: strings_opt[1] = describe.replace('[]', '[' + value[0] + ']')
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', '[' + value[rid] + ']', 1)
                        strings_opt[-1] = describe
                    elif bracket_num == 1:
                        strings_opt[-1] = describe.replace('[]', '[' + strings_opt[-1] + ']')
                    elif describe != '': strings_opt[0] = strings_opt[0] + describe
                    strings_log = [strings_opt[0], strings_opt[-1]]
                case 'MAJ':
                    if roll_num == 1 and bracket_num != 1: strings_opt[0] = strings_opt[0] + describe
                    elif roll_num == 1 and bracket_num == 1: strings_opt[1] = describe.replace('[]', '[' + value[0] + ']')
                ...
                case 'GUA':
                    if roll_num == 1 and bracket_num != 1: strings_opt[0] = strings_opt[0] + describe
                    elif roll_num == 1 and bracket_num == 1: strings_opt.append(describe.replace('[]', f'[{value[0][2]}({value[0][3]},{value[0][4]})]'))
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', f'[{value[rid][2]}({value[rid][3]},{value[rid][4]})]', 1)
                        strings_opt[-1] = describe
                    elif bracket_num == 1:
                        strings_opt[-1] = strings_opt[-1].replace(' | ', ',').replace('Results:,', '')
                        strings_opt[-1] = describe.replace('[]', '[' + strings_opt[-1][:-2] + ']')
    ...
    output_create_record(len(strings_opt), value, code, display_mode)
    create_log(strings_log, code)
    output_totext(strings_opt, display_mode)
@protect
def show_result_nopro_base(ipt: str, code: str):
    if not iflegal_branch(ipt, code): return
    ...
    match code: # 输出文本特殊处理, 包含描述的插入
        ...
        case 'against':
            ...
            else: strings_opt = ['|' + ipt_std + '| = ' + string_0]
            strings_log = [strings_opt[0]]
        case 'special':
            _, order, roll_num = iflegal_special(ipt)
            match order:
                ...
                case 'POK':
                    if roll_num == 1 and describe == '': strings_opt = ['随机扑克牌: ' + value[0]]
                    elif roll_num == 1 and bracket_num != 1: strings_opt = [describe + ': ' + value[0]]
                    elif roll_num == 1 and bracket_num == 1: strings_opt = [describe.replace('[]', '[' + value[0] + ']')]
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', '[' + value[rid] + ']', 1)
                        strings_opt = [describe]
                    elif bracket_num == 1:
                        strings_opt = [describe.replace('[]', '[ ' + ' | '.join(value) + ' ]')]
                    elif describe != '': strings_opt = [describe + ': | ' + ' | '.join(value) + ' |']
                    else: strings_opt = ['随机扑克牌: | ' + ' | '.join(value) + ' |']
                case 'MAJ':
                    if roll_num == 1 and describe == '': strings_opt = ['随机麻将牌: ' + value[0]]
                    elif roll_num == 1 and bracket_num != 1: strings_opt = [describe + ': ' + value[0]]
                    elif roll_num == 1 and bracket_num == 1: strings_opt = [describe.replace('[]', '[' + value[0] + ']')]
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', '[' + value[rid] + ']', 1)
                    ...
                    else: strings_opt = ['随机麻将牌: | ' + ' | '.join(value) + ' |']
                case 'GUA':
                    gua_ranks, gua_names = [gua_64_rank[avalue[0]][avalue[1]] for avalue in value], [gua_64[avalue[0]][avalue[1]] for avalue in value]
                    gua_unicodes = [gua_64_unicode[arank - 1] for arank in gua_ranks]
                    entire_strings = [(f'{avalue[0]}({avalue[1]},{avalue[2]})') for avalue in zip(gua_names, gua_ranks, gua_unicodes)]
                    if roll_num == 1 and describe == '': strings_opt = ['随机六十四卦: ' + entire_strings[0]]
                    elif roll_num == 1 and bracket_num != 1: strings_opt = [describe + ': ' + entire_strings[0]]
                    elif roll_num == 1 and bracket_num == 1: strings_opt = [describe.replace('[]', '[' + entire_strings[0] + ']')]
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', '[' + entire_strings[rid] + ']', 1)
                        strings_opt = [describe]
                    elif bracket_num == 1:
                        strings_opt = [describe.replace('[]', '[ ' + ' | '.join(entire_strings) + ' ]')]
                    elif describe != '': strings_opt = [describe + ': | ' + ' | '.join(entire_strings) + ' |']
                    else: strings_opt = ['随机六十四卦: | ' + ' | '.join(entire_strings) + ' |']
                case 'TLL':
                    if roll_num == 1 and describe == '': strings_opt = ['随机塔罗牌: ' + value[0]]
                    elif roll_num == 1 and bracket_num != 1: strings_opt = [describe + ': ' + value[0]]
                    elif roll_num == 1 and bracket_num == 1: strings_opt = [describe.replace('[]', '[' + value[0] + ']')]
                    elif bracket_num == roll_num:
                        for rid in range(bracket_num): describe = describe.replace('[]', '[' + value[rid] + ']', 1)
...
def showcolors(colorss: list[list[str]]):
    '''展示随机RGB所得颜色'''
    ...
    for lid in range(length_total):
        ...
        for cid in range(length):
            ...
            canvas_showcolor.create_rectangle(left, top, WIDTH_COLORSHOW, HEIGHT_COLORSHOW, fill = colorss[lid][cid])
def embed(ipt: str, code: str) -> str:
    if code not in codes or not iflegal_branch(ipt, code): return ''
    value, ipt_std = nopro_branch(ipt, code), standardization(ipt, code)
    match code:
        case 'classic': return ipt_std + f'={value}'
        case 'D__func': return ipt_std + '=' + ','.join([str(avalue) for avalue in value])
        case 'if_true': return ipt_std + f':{value[0]}{value[2]}{value[1]},{value[3]}'
        case 'against': return '|'.join([f'{ipt}={value}' for ipt, value in zip(split(['|'], ipt_std), [nopro_dice(ipt) for ipt in split(['|'], ipt_std)])])
        case 'special':
            _, order, _ = iflegal_special(ipt)
            match order:
                case 'RGB': return ','.join([RGB_hex(avalue) for avalue in value])
                case 'POK': return ','.join(value)
                case 'MAJ': return ','.join(value)
                case 'GUA': return ','.join([gua8_2_gua64(avalue[0], avalue[1]) for avalue in value])
                case 'TLL': return ','.join(value)
                case 'TLA': return ','.join(value)
@protect
def direct_describe():
    describe = describe_get()
    clear_describe()
    ifmatch, L_mainbracket, R_mainbracket = squarebracket_match(describe)
    if not ifmatch or len(L_mainbracket) == 0:
        ...
        embed_strings = [embed(ipt, code) for ipt, code in zip(contents, codes)]
        for cid in range(len(codes) - 1, -1, -1): describe = describe[:L_mainbracket[cid] + 1] + embed_strings[cid] + describe[R_mainbracket[cid]:]
        output_create_record(1, embed_strings, codes, False)
        colorss = []
        for cid in range(len(codes)):
            if codes[cid] == 'special' and contents[cid][:3] == 'RGB':
                colorss.append(embed_strings[cid].split(','))
        if len(colorss) > 0: showcolors(colorss)
    create_log([describe], 'direct')
    output_totext([describe], False)
...
text_describe.bind('<Return>', lambda event: direct_describe() or 'break')
...
buttons_inputs = [Button(text_input_control_frame, text = ipt_texts[bid], font = FONT, command = ipt_funcs[bid]) for bid in range(INPUT_BUTTON_NUM)]
